...
import {
  ...
  ChevronRight, Loader2, RefreshCw, CarFront, Bike, Package, Truck, Wallet,
  ChevronDown,
  ArrowLeft
} from "lucide-react";
...
export default function PartnerBookingsPage() {
  ...
  useEffect(() => {
    ...
    // Auto-refresh when user completes payment or cancels
    newSocket.on("ride_updated", () => {
      showAlert("Payment received! The ride is now confirmed.", "success");
      fetchBookings();
    });
    newSocket.on("ride_cancelled", () => fetchBookings());
  ...
  const filteredBookings = bookings.filter(b => {
    ...
    if (filter === "ACTIVE") return ["AWAITING_PAYMENT", "CONFIRMED", "STARTED"].includes(b.bookingStatus);
  ...
  return (
    <main className="min-h-dvh bg-gray-50 dark:bg-[#050505] font-sans transition-colors duration-300 [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
      <div className="sticky top-0 z-50 pb-12 w-full bg-black text-white shadow-md">
        <Navbar onLoginClick={() => { }} />
      </div>
      ...
      <div className="max-w-[800px] mx-auto px-4 sm:px-6 pb-12 md:pb-20">
          ...
          <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} className="flex items-start gap-4">
            ...
            <div>
              <div className="flex items-center gap-3 mb-2">
                ...
                <h1 className="text-3xl font-black text-black dark:text-white tracking-tight">Earnings & Rides</h1>
              </div>
              <p className="text-gray-500 dark:text-gray-400 font-medium ml-1">{bookings.length} assignments found</p>
            </div>
          </motion.div>
          <motion.div
        ...
        {isLoading ? (
          ...
          <div className="flex flex-col items-center justify-center py-20 bg-white dark:bg-[#0a0a0a] rounded-3xl border border-gray-100 dark:border-gray-900 shadow-sm text-center px-6">
            ...
            <h3 className="font-bold text-black dark:text-white mb-1">No Assignments Yet</h3>
            <p className="text-xs text-gray-500">When you accept rides, they will appear here.</p>
          </div>
        ) : (
          <div className="space-y-6">
            <AnimatePresence mode="popLayout">
              {filteredBookings.map((booking, i) => {
                ...
                // ONLY show the tracking/start button if payment is confirmed
                const isReadyToStart = ["CONFIRMED", "STARTED"].includes(booking.bookingStatus);
                  ...
                  >
                    ...
                    <div className="p-6 border-b border-gray-50 dark:border-gray-900/50 flex justify-between items-center bg-gray-50/50 dark:bg-gray-900/20">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <h3 className="font-black text-sm text-black dark:text-white uppercase tracking-wider">{booking.user?.name || "Rider"}</h3>
                          <div className="flex items-center text-gray-500 dark:text-gray-400 text-[10px] mt-0.5 font-bold tracking-widest gap-1">
                            <Phone className="w-3 h-3" /> {booking.userMobileNumber}
                          </div>
                        </div>
                      </div>
                    ...
                    <div className="p-6">
                      <div className="flex items-center gap-2 mb-6">
                        ...
                        <span className="text-xs font-bold text-gray-500 dark:text-gray-400 uppercase tracking-widest">{booking.vehicle?.vehicleNumber}</span>
                      </div>
                    ...
                    <div className="px-6 py-4 bg-gray-50 dark:bg-gray-900/40 flex items-center justify-between">
                      ...
                      <div className="flex items-center gap-4">
                        <div className="text-right">
                          <p className="text-[9px] font-black text-gray-400 uppercase tracking-widest">Your Earnings</p>
                          <span className="text-xl font-black text-black dark:text-white tracking-tight">₹{booking.partnerAmount}</span>
                        </div>
                        {isReadyToStart && (
                          <button
                            onClick={() => router.push(`/partner/ride/active?id=${booking.id}`)}
                            className="flex items-center gap-1 text-xs font-bold text-purple-600 hover:text-purple-700 dark:text-purple-400 dark:hover:text-purple-300 transition-colors"
                          >
                            Start Ride <ChevronRight className="w-4 h-4" />
                          </button>
                        )}
                      </div>
                    </div>
