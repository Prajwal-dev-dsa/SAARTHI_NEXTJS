...
export default function Navbar({ onLoginClick }: NavbarProps) {
  ...
  // --- Fetch Pending Requests Count for Partners ---
  useEffect(() => {
    if (isAuthenticated && user?.role === "PARTNER") {
      ...
      fetchPendingCount();
      const intervalId = setInterval(fetchPendingCount, 15000); // Poll every 15 seconds
  ...
  const PARTNER_LINKS: NavLink[] = [
    ...
    { name: "Active Ride", href: "/partner/ride/active" },
  ];
  ...
  return (
    <nav className="absolute top-0 left-0 w-full z-50 flex items-center justify-between px-6 py-6 md:px-16 text-black dark:text-white bg-transparent transition-colors duration-300">

      {/* LOGO */}
      <div
        onClick={() => router.push("/")}
        className="text-2xl font-black tracking-widest uppercase cursor-pointer"
      >
        Saarthi
      </div>
        ...
        {linksToRender.map((link) => (
          <div key={link.name} className="relative flex items-center">
            <span
              ...
              className="hover:text-gray-600 dark:hover:text-gray-300 cursor-pointer transition-colors relative flex items-center"
            >
              {link.name}

              {link.showBadge && pendingCount > 0 && (
                <span className="ml-1.5 bg-red-600 border border-white dark:border-[#050505] text-white text-[10px] font-black w-5 h-5 flex items-center justify-center rounded-full shadow-lg">
                  {pendingCount}
                </span>
              )}
            </span>
        ...
        {isAuthenticated ? (
          <div className="relative" ref={profileRef}>
            <button
              ...
              className="w-10 h-10 rounded-full bg-black dark:bg-white text-white dark:text-black flex items-center justify-center font-bold text-lg hover:ring-4 hover:ring-black/20 dark:hover:ring-white/30 transition-all"
            >
              {userInitial}
            </button>
            ...
            <AnimatePresence>
              {isProfileOpen && (
                <motion.div
                  ...
                  className="absolute right-0 mt-4 w-72 bg-white dark:bg-[#0a0a0a] border border-gray-100 dark:border-gray-800 rounded-3xl p-5 shadow-2xl origin-top-right"
                >
                  <ProfileMenuContent />
                </motion.div>
              )}
          ...
          <button
            ...
            className="bg-black dark:bg-white text-white dark:text-black px-6 py-2.5 rounded-full font-bold text-sm hover:scale-105 transition-transform shadow-md"
          >
            Login
          </button>
        )}
      ...
      <div className="md:hidden flex items-center space-x-4">
        {isAuthenticated ? (
          <button
            ...
            className="w-10 h-10 rounded-full bg-black dark:bg-white text-white dark:text-black flex items-center justify-center font-bold text-lg"
          >
            {userInitial}
          </button>
        ) : (
          <button
            onClick={onLoginClick}
            className="bg-black dark:bg-white text-white dark:text-black px-4 py-1.5 rounded-full font-bold text-sm"
          >
            Login
          </button>
        )}
        <button onClick={() => setIsMobileMenuOpen(true)} className="relative">
          ...
          {user?.role === "PARTNER" && pendingCount > 0 && (
            <div className="absolute -top-1 -right-1 w-3 h-3 bg-red-600 rounded-full border-2 border-white dark:border-[#050505]" />
          )}
        </button>
      </div>
